  ...
  var init_default = __esm({
    "src/core/i18n/default.json"() {
      default_default = {
        ...
        BYPASS_ALL: "Bypass All Restrictions",
        BYTECODE: "Bytecode",
        CANCEL: "Cancel",
        CLEAR: "Clear",
        ...
        CLIPBOARD_EMPTY: "Clipboard is empty or contains no valid plugin data.",
        CODEBERG: "Codeberg",
        CODENAME: "Codename",
        ...
        COPY_URL: "Copy URL",
        CUSTOM_PLUGIN_HISTORY: "Custom Plugin History",
        DEBUG: "Debug",
        ...
        ENTER_FILE_PATH_DESC: "Enter the full path to the plugin file (.js) on your device.",
        ...
        IMPORT_LOCAL_FILE_DESC: "Import a plugin from a .js file on your device. You can enter the file path or browse your files.",
        INFO: "Info",
        INSTALL: "Install",
        ...
        LOAD_FROM_URL_DESC: "Enter the direct URL to a .js file (e.g., GitHub raw, Codeberg raw). The plugin code will be fetched and loaded immediately.",
        ...
        OPEN_PLUGIN_LOADER: "Open Plugin Loader",
        OPEN_PLUGIN_LOADER_DESC: "Load plugins from local files, URLs, or paste code directly. No repository or proxy restrictions.",
        OPEN_PLUGIN_LOADER_SETTINGS_DESC: "Manage custom-loaded plugins, import from files, URLs or paste code.",
        OPEN_PLUGIN_REMOVE_DESC: "This will completely remove the plugin, its code, and all associated data. This action cannot be undone.",
        OPEN_PLUGIN_SETTINGS: "Open Plugin Loader (Kettu-OpenLoader)",
        OPEN_PLUGIN_WARNING: "Warning: Custom plugins run arbitrary code in your Discord client. Only load plugins from sources you trust. Malicious plugins can steal your data, tokens, or compromise your account.",
        OPERATING_SYSTEM: "Operating System",
        OVERFLOW_PLUGIN_SETTINGS: "Plugin Settings",
        PASTE_PLUGIN_CODE: "Paste Plugin Code",
        PASTE_PLUGIN_CODE_DESC: "Paste the raw JavaScript code of a plugin. The code will be loaded and executed directly.",
        PLATFORM: "Platform",
        PLUGIN_NAME_OPTIONAL: "Plugin name (optional)",
        ...
        SAFE_MODE_NOTICE_THEMES: "You are in safe mode, meaning themes have been temporarily disabled. {enabled, select, true {If a font appears to be causing the issue, you can press below to disable it persistently.} other {}}",
        SEARCH: "Search",
        SEPARATOR: ", ",
        SETTINGS_ACTIVATE_DISCORD_EXPERIMENTS: "Activate Discord Experiments",
        SETTINGS_ACTIVATE_DISCORD_EXPERIMENTS_DESC: "Warning: Messing with this feature may lead to account termination. I heavily discourage using this and am not responsible for anything that happens if you use this",
        ...
        UNRESTRICTED_PLUGIN_LOADING: "Unrestricted Plugin Loading",
        UNRESTRICTED_PLUGIN_LOADING_DESC: "Bypass all proxy/whitelist restrictions. Allows loading plugins from any source without confirmation dialogs.",
        ...
        evalPlugin(plugin) {
          return _async_to_generator(function* () {
            var vendettaForPlugins = {
              ...
              logger: new LoggerClass(`Kettu-OpenLoader \xBB ${plugin.manifest.name}`)
            };
...
//# sourceURL=${plugin.id}`;
  ...
  __export(plugins_exports2, {
    ...
    customPluginHistory: () => customPluginHistory,
    deleteRepository: () => deleteRepository,
    disablePlugin: () => disablePlugin,
    enablePlugin: () => enablePlugin,
    generateCustomPluginId: () => generateCustomPluginId,
    ...
    loadCustomPlugin: () => loadCustomPlugin,
    loadPluginFromCode: () => loadPluginFromCode,
    loadPluginFromFile: () => loadPluginFromFile,
    ...
    reloadCustomPlugin: () => reloadCustomPlugin,
    removeCustomPlugin: () => removeCustomPlugin,
    restoreCustomPlugins: () => restoreCustomPlugins,
  ...
  function isCustomPlugin(manifest) {
    return "source" in manifest && [
      "file",
      "url",
      "paste"
    ].includes(manifest.source);
  }
  ...
  function generateCustomPluginId(nameOrUrl) {
    var sanitized = nameOrUrl.replace(/^https?:\/\//, "").replace(/[^a-zA-Z0-9._-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "").slice(0, 48);
    var id = `custom.${sanitized}`;
    var finalId = id;
    var counter = 1;
    while (registeredPlugins.has(finalId) || isPluginInstalled(finalId)) {
      finalId = `${id}-${counter++}`;
    }
    return finalId;
  }
  function loadCustomPlugin(jsCode, options) {
    return _async_to_generator(function* () {
      ...
      var pluginName = options.name || "Custom Plugin";
      var nameMatch = jsCode.match(/name:\s*["']([^"']+)["']/);
      if (nameMatch && !options.name)
        pluginName = nameMatch[1];
      var jsPath = `plugins/custom/${id}.js`;
      ...
      yield writeFile(jsPath, jsCode);
      registeredPlugins.set(id, manifest);
      customPluginHistory[id] = {
        id,
        manifest,
        enabled: true,
        js: jsCode
  ...
  function loadPluginFromFile(filePath, name) {
    return _async_to_generator(function* () {
      var jsCode;
      try {
        jsCode = yield readFile(filePath);
      } catch (error) {
  ...
  function loadPluginFromUrl(url2, name) {
    return _async_to_generator(function* () {
  ...
  function loadPluginFromCode(code, name) {
    return _async_to_generator(function* () {
      return loadCustomPlugin(code, {
        name,
        source: "paste",
        sourceInfo: (/* @__PURE__ */ new Date()).toISOString()
      });
    })();
  }
  function removeCustomPlugin(id) {
    return _async_to_generator(function* () {
      ...
      if (!manifest || !isCustomPlugin(manifest)) {
        throw new Error(`Plugin '${id}' is not a custom plugin or is not registered`);
      }
      ...
      delete customPluginHistory[id];
      try {
        yield removeFile(`plugins/custom/${id}.js`);
      } catch (e) {
      }
      try {
  ...
  function getCustomPlugins() {
    return Object.values(customPluginHistory);
  }
  function reloadCustomPlugin(id) {
    return _async_to_generator(function* () {
      var record = customPluginHistory[id];
      if (!record)
        throw new Error(`Custom plugin '${id}' not found in history`);
      if (pluginInstances.has(id))
        stopPlugin(id);
      yield writeFile(record.manifest.jsPath, record.js);
      registeredPlugins.set(id, record.manifest);
      pluginSettings[id] = {
  ...
  function restoreCustomPlugins() {
    return _async_to_generator(function* () {
      yield awaitStorage2(customPluginHistory);
      var records = Object.values(customPluginHistory);
      if (records.length === 0)
        return;
      for (var record of records) {
        try {
          registeredPlugins.set(record.id, record.manifest);
          yield writeFile(record.manifest.jsPath, record.js);
          pluginSettings[record.id] = {
            enabled: record.enabled
          };
          if (record.enabled) {
            startPlugin(record.id, {
              disableWhenThrown: false
            });
          }
        } catch (error) {
          console.error(`[Kettu-OpenLoader] Failed to restore custom plugin '${record.id}':`, error);
          delete customPluginHistory[record.id];
        }
      }
    })();
  }
  ...
  function refreshPlugin(id, repoUrl) {
    return _async_to_generator(function* () {
      ...
      stopPlugin(id);
      if (isCustomPlugin(manifest)) {
        yield reloadCustomPlugin(id);
        return;
      }
  ...
  function startPlugin(_0) {
    return _async_to_generator(function* (id, { throwIfDisabled = false, disableWhenThrown = true } = {}) {
      ...
      if (isCorePlugin(id)) {
        ...
        var jsPath = isCustomPlugin(manifest) ? manifest.jsPath : manifest.jsPath;
        assert(jsPath, id, "start a plugin with no JS path");
        try {
          var iife = yield readFile(jsPath);
          var instantiator = globalEvalWithSourceUrl(`(bunny,definePlugin)=>{${iife};return plugin?.default ?? plugin;}`, `kettu-openloader/${id}-${manifest.version}`);
        } catch (error) {
  ...
  var corePluginInstances, registeredPlugins, pluginInstances, apiObjects, pluginRepositories, pluginSettings, customPluginHistory, _fetch, fetchJS, fetchJSON;
  var init_plugins4 = __esm({
    "src/lib/addons/plugins/index.ts"() {
      ...
      customPluginHistory = createStorage2("plugins/custom-history.json");
              ...
              children: [
                ...
                /* @__PURE__ */ jsx(import_react_native23.View, {
                  ...
                  children: /* @__PURE__ */ jsx(IconButton, {
                    ...
                    onPress: () => {
                      var { NavigationNative: NavigationNative2 } = (init_common(), __toCommonJS(common_exports));
                      var navigation2 = NavigationNative2.useNavigation();
                      navigation2.push("PUPU_CUSTOM_PAGE", {
  ...
  // src/core/ui/settings/pages/Plugins/LoadCustomPlugin.tsx
  var LoadCustomPlugin_exports = {};
  __export(LoadCustomPlugin_exports, {
    default: () => LoadCustomPlugin
  });
  function LoadCustomPlugin() {
    useProxy(settings);
    var navigation2 = NavigationNative.useNavigation();
    var [pluginList, setPluginList] = (0, import_react8.useState)([]);
    var refreshList = (0, import_react8.useCallback)(() => {
      setPluginList([
        ...getCustomPlugins()
      ]);
    }, []);
          ...
          /* @__PURE__ */ jsxs(import_react_native30.View, {
            children: [
              /* @__PURE__ */ jsx(Text, {
                variant: "heading-lg/bold",
                color: "text-default",
                children: Strings.OPEN_PLUGIN_LOADER
              }),
              /* @__PURE__ */ jsx(Text, {
                variant: "text-sm/medium",
                color: "text-muted",
                ...
                children: Strings.OPEN_PLUGIN_LOADER_DESC
              })
            ]
          }),
          /* @__PURE__ */ jsx(Card, {
            ...
            children: /* @__PURE__ */ jsx(import_react_native30.View, {
              ...
              children: /* @__PURE__ */ jsx(Text, {
                ...
                children: Strings.OPEN_PLUGIN_WARNING
              })
            })
          }),
          /* @__PURE__ */ jsxs(Stack, {
            ...
            children: [
              /* @__PURE__ */ jsx(Button, {
                size: "lg",
                icon: findAssetId("FileIcon"),
                text: Strings.IMPORT_LOCAL_FILE,
                variant: "primary",
                onPress: () => openFilePickerAlert(refreshList)
              }),
              /* @__PURE__ */ jsx(Button, {
                size: "lg",
                icon: findAssetId("LinkIcon"),
                text: Strings.LOAD_FROM_URL,
                variant: "secondary",
                onPress: () => openUrlInputAlert(refreshList)
              }),
              /* @__PURE__ */ jsx(Button, {
                size: "lg",
                icon: findAssetId("PencilIcon"),
                text: Strings.PASTE_PLUGIN_CODE,
                variant: "secondary",
                onPress: () => openPasteCodeAlert(refreshList)
              }),
              /* @__PURE__ */ jsx(Button, {
                size: "lg",
                icon: findAssetId("ClipboardListIcon"),
                ...
                variant: "secondary",
                onPress: () => importFromClipboard(refreshList)
              })
            ]
          }),
          pluginList.length > 0 && /* @__PURE__ */ jsxs(import_react_native30.View, {
            children: [
              /* @__PURE__ */ jsxs(Stack, {
                spacing: 8,
                style: {
                ...
                children: [
                  /* @__PURE__ */ jsx(Text, {
                    variant: "heading-md/bold",
                    color: "text-default",
                    children: Strings.CUSTOM_PLUGIN_HISTORY
                  }),
                  /* @__PURE__ */ jsxs(Text, {
                    variant: "text-sm/medium",
                    color: "text-muted",
                    children: [
                      pluginList.length,
                      " ",
                      pluginList.length === 1 ? "plugin" : "plugins"
                    ]
                  })
                ]
              }),
              /* @__PURE__ */ jsx(FlashList, {
                ...
                renderItem: ({ item }) => /* @__PURE__ */ jsx(CustomPluginCard, {
                  record: item,
  ...
  function CustomPluginCard({ record, onRefresh }) {
    var [enabled, setEnabled] = (0, import_react8.useState)(isPluginEnabled(record.id));
    var [loading, setLoading] = (0, import_react8.useState)(false);
    var sourceLabel = {
      file: `File: ${record.manifest.sourceInfo?.split("/").pop() || "Unknown"}`,
      url: `URL: ${record.manifest.sourceInfo || "Unknown"}`,
      paste: `Pasted at ${record.manifest.loadedAt ? new Date(record.manifest.loadedAt).toLocaleString() : "Unknown"}`
    }[record.manifest.source] || "Unknown source";
    var handleToggle = (v2) => _async_to_generator(function* () {
      setLoading(true);
      try {
        if (v2) {
          yield enablePlugin(record.id, true);
        } else {
        ...
        logger.error(`Failed to toggle custom plugin '${record.id}':`, e);
        showToast(`Failed: ${e instanceof Error ? e.message : String(e)}`, findAssetId("Small"));
      } finally {
        setLoading(false);
      }
    })();
    var handleReload = () => _async_to_generator(function* () {
      setLoading(true);
      try {
        yield reloadCustomPlugin(record.id);
        showToast(`${Strings.PLUGIN_REFETCH_SUCCESSFUL} (reloaded)`, findAssetId("Check"));
        onRefresh();
      } catch (e) {
        logger.error(`Failed to reload custom plugin '${record.id}':`, e);
        showToast(`Reload failed: ${e instanceof Error ? e.message : String(e)}`, findAssetId("Small"));
      } finally {
        setLoading(false);
      }
    })();
    var handleRemove = () => {
      openAlert4(`remove-custom-${record.id}`, /* @__PURE__ */ jsx(AlertModal, {
        title: Strings.ARE_YOU_SURE_TO_DELETE_PLUGIN.replace("{name}", record.manifest.display.name),
        content: Strings.OPEN_PLUGIN_REMOVE_DESC,
        actions: /* @__PURE__ */ jsxs(Stack, {
          children: [
            /* @__PURE__ */ jsx(AlertActionButton, {
              text: Strings.DELETE,
              variant: "destructive",
              onPress: () => _async_to_generator(function* () {
                dismissAlert2(`remove-custom-${record.id}`);
                try {
                  yield removeCustomPlugin(record.id);
                  showToast(`${record.manifest.display.name} removed`, findAssetId("TrashIcon"));
                  onRefresh();
                } catch (e) {
                  logger.error(`Failed to remove custom plugin:`, e);
                  showToast(`Remove failed: ${e instanceof Error ? e.message : String(e)}`, findAssetId("Small"));
                }
              })()
            }),
            /* @__PURE__ */ jsx(AlertActionButton, {
    ...
    return /* @__PURE__ */ jsx(Card, {
      children: /* @__PURE__ */ jsxs(Stack, {
        ...
        children: [
              ...
              /* @__PURE__ */ jsxs(import_react_native30.View, {
                ...
                children: [
                  /* @__PURE__ */ jsx(Text, {
                    variant: "heading-md/semibold",
                    numberOfLines: 1,
                    children: record.manifest.display.name
                  }),
                  /* @__PURE__ */ jsxs(Text, {
                    variant: "text-xs/medium",
                    color: "text-muted",
          ...
          record.manifest.display.description && /* @__PURE__ */ jsx(Text, {
            variant: "text-sm/medium",
            color: "text-muted",
            numberOfLines: 2,
            children: record.manifest.display.description
          })
        ]
      })
    });
  }
  function openUrlInputAlert(onSuccess) {
    var url2 = "";
    var name = "";
    var error = "";
    var loading = false;
    var key = "kettu-openloader-url";
    var render = () => /* @__PURE__ */ jsx(AlertModal, {
      title: Strings.LOAD_FROM_URL,
      content: Strings.LOAD_FROM_URL_DESC,
      extraContent: /* @__PURE__ */ jsxs(Stack, {
        spacing: 12,
        style: {
          marginTop: -12
        ...
        children: [
          /* @__PURE__ */ jsx(TextInput, {
            autoFocus: true,
            isClearable: true,
            placeholder: "https://raw.githubusercontent.com/user/plugin/main/index.js",
            defaultValue: url2,
            onChange: (v2) => {
              url2 = v2;
              error = "";
          ...
          /* @__PURE__ */ jsx(TextInput, {
            placeholder: Strings.PLUGIN_NAME_OPTIONAL,
            defaultValue: name,
            onChange: (v2) => {
              name = v2;
              render();
      ...
      actions: /* @__PURE__ */ jsxs(Stack, {
        children: [
          /* @__PURE__ */ jsx(Button, {
            ...
            variant: "primary",
            disabled: !url2 || !isValidHttpUrl(url2),
            onPress: () => _async_to_generator(function* () {
              loading = true;
              render();
              try {
                yield loadPluginFromUrl(url2, name || void 0);
                dismissAlert2(key);
                showToast(Strings.TOASTS_INSTALLED_PLUGIN, findAssetId("Check"));
                onSuccess();
              } catch (e) {
          ...
          /* @__PURE__ */ jsx(AlertActionButton, {
            ...
            variant: "secondary",
            disabled: loading,
            onPress: () => dismissAlert2(key)
          })
        ]
      })
    });
    openAlert4(key, render());
  }
  function openPasteCodeAlert(onSuccess) {
    var code = "";
    var name = "";
    var error = "";
    var loading = false;
    var key = "kettu-openloader-paste";
    var render = () => /* @__PURE__ */ jsx(AlertModal, {
      title: Strings.PASTE_PLUGIN_CODE,
      content: Strings.PASTE_PLUGIN_CODE_DESC,
      extraContent: /* @__PURE__ */ jsxs(Stack, {
        spacing: 12,
        style: {
          marginTop: -12
        ...
        children: [
          /* @__PURE__ */ jsx(Button, {
            ...
            onPress: () => _async_to_generator(function* () {
              ...
              render();
              try {
                yield loadPluginFromCode(code, name || void 0);
                dismissAlert2(key);
                showToast(Strings.TOASTS_INSTALLED_PLUGIN, findAssetId("Check"));
                onSuccess();
              } catch (e) {
  ...
  function importFromClipboard(onSuccess) {
    return _async_to_generator(function* () {
      try {
        var content = yield clipboard.getString();
        if (!content || content.trim().length === 0) {
          showToast(Strings.CLIPBOARD_EMPTY, findAssetId("Small"));
          return;
        }
        if (isValidHttpUrl(content)) {
          yield loadPluginFromUrl(content);
        } else {
          yield loadPluginFromCode(content);
        }
        showToast(Strings.TOASTS_INSTALLED_PLUGIN, findAssetId("Check"));
        onSuccess();
      } catch (e) {
        logger.error("Failed to import from clipboard:", e);
        showToast(`Import failed: ${e instanceof Error ? e.message : String(e)}`, findAssetId("Small"));
      }
    })();
  }
  function openFilePickerAlert(onSuccess) {
    var key = "kettu-openloader-file";
    openAlert4(key, /* @__PURE__ */ jsx(AlertModal, {
      title: Strings.IMPORT_LOCAL_FILE,
      content: Strings.IMPORT_LOCAL_FILE_DESC,
      actions: /* @__PURE__ */ jsxs(Stack, {
        children: [
          /* @__PURE__ */ jsx(AlertActionButton, {
            text: Strings.BROWSE_FILES,
            variant: "primary",
            onPress: () => _async_to_generator(function* () {
              dismissAlert2(key);
              try {
                yield openFilePathInput(onSuccess);
              } catch (e) {
                logger.error("File picker failed:", e);
                showToast(`File import failed: ${e instanceof Error ? e.message : String(e)}`, findAssetId("Small"));
              }
            })()
          }),
          /* @__PURE__ */ jsx(AlertActionButton, {
            text: Strings.ENTER_PATH,
            variant: "secondary",
            onPress: () => {
              dismissAlert2(key);
              openFilePathInput(onSuccess);
            }
          }),
          /* @__PURE__ */ jsx(AlertActionButton, {
            text: Strings.CANCEL,
  ...
  function openFilePathInput(onSuccess) {
    var path = "";
    var name = "";
    var error = "";
    var loading = false;
    var key = "kettu-openloader-filepath";
    var render = () => /* @__PURE__ */ jsx(AlertModal, {
      title: Strings.ENTER_FILE_PATH,
      content: Strings.ENTER_FILE_PATH_DESC,
      extraContent: /* @__PURE__ */ jsxs(Stack, {
        spacing: 12,
        style: {
          marginTop: -12
        ...
        children: [
          /* @__PURE__ */ jsx(Button, {
            ...
            onPress: () => _async_to_generator(function* () {
              ...
              render();
              try {
                yield loadPluginFromFile(path, name || void 0);
                dismissAlert2(key);
                showToast(Strings.TOASTS_INSTALLED_PLUGIN, findAssetId("Check"));
                onSuccess();
              } catch (e) {
  ...
  var import_react8, import_react_native30, openAlert4, dismissAlert2, showSimpleActionSheet5, hideActionSheet4;
  var init_LoadCustomPlugin = __esm({
    "src/core/ui/settings/pages/Plugins/LoadCustomPlugin.tsx"() {
      "use strict";
      init_asyncIteratorSymbol();
      init_promiseAllSettled();
      ...
      ({ openAlert: openAlert4, dismissAlert: dismissAlert2 } = lazyDestructure(() => findByProps("openAlert", "dismissAlert")));
      ({ showSimpleActionSheet: showSimpleActionSheet5, hideActionSheet: hideActionSheet4 } = lazyDestructure(() => findByProps("showSimpleActionSheet")));
    }
  });
  ...
  function AssetDisplay({ asset }) {
    return /* @__PURE__ */ jsx(TableRow, {
      ...
      onPress: () => openAlert5("revenge-asset-display-details", /* @__PURE__ */ jsx(AlertModal5, {
        title: asset.name,
...
Type: ${asset.type}`,
  ...
  var import_react_native31, openAlert5, AlertModal5, AlertActionButton5, displayable, iconMap, copyToClipboard;
      ...
      children: /* @__PURE__ */ jsx(import_react_native34.ScrollView, {
        ...
        children: /* @__PURE__ */ jsxs(Stack, {
          ...
          children: [
            ...
            /* @__PURE__ */ jsxs(TableRowGroup, {
              title: Strings.OPEN_PLUGIN_SETTINGS,
              children: [
                /* @__PURE__ */ jsx(TableSwitchRow, {
                  label: Strings.UNRESTRICTED_PLUGIN_LOADING,
                  subLabel: Strings.UNRESTRICTED_PLUGIN_LOADING_DESC,
                  icon: /* @__PURE__ */ jsx(TableRow.Icon, {
                    source: findAssetId("WrenchIcon")
                  }),
                  value: settings.unrestrictedPluginLoading ?? true,
                  onValueChange: (v2) => {
                    settings.unrestrictedPluginLoading = v2;
                  }
                }),
                /* @__PURE__ */ jsx(TableRow, {
                  arrow: true,
                  label: Strings.OPEN_PLUGIN_LOADER,
                  subLabel: Strings.OPEN_PLUGIN_LOADER_SETTINGS_DESC,
                  icon: /* @__PURE__ */ jsx(TableRow.Icon, {
                    source: findAssetId("AppsIcon")
                  }),
                  trailing: TableRow.Arrow,
                  onPress: () => navigation2.push("PUPU_CUSTOM_PAGE", {
                    title: Strings.OPEN_PLUGIN_LOADER,
                    render: LoadCustomPlugin
                  })
                })
              ]
            }),
            /* @__PURE__ */ jsxs(TableRowGroup, {
              ...
              children: [
                /* @__PURE__ */ jsx(TableRow, {
                  ...
                  onPress: () => {
                    openAlert6("pupu-clear-bundle-reload-confirmation", /* @__PURE__ */ jsx(AlertModal6, {
                ...
                /* @__PURE__ */ jsx(TableRow, {
                  ...
                  onPress: () => showSimpleActionSheet6({
  ...
  var import_react_native33, import_react_native34, import_react11, hideActionSheet5, showSimpleActionSheet6, openAlert6, AlertModal6, AlertActionButton6, RDT_EMBED_LINK, useStyles4;
  var init_Developer = __esm({
    "src/core/ui/settings/pages/Developer/index.tsx"() {
      ...
      ({ hideActionSheet: hideActionSheet5 } = lazyDestructure(() => findByProps("openLazy", "hideActionSheet")));
      ({ showSimpleActionSheet: showSimpleActionSheet6 } = lazyDestructure(() => findByProps("showSimpleActionSheet")));
  ...
  function initSettings() {
    ...
    registerSection({
      ...
      items: [
        {
          key: "OPENLOADER_MANAGE",
          title: () => Strings.OPEN_PLUGIN_LOADER,
          icon: findAssetId("AppsIcon"),
          render: () => Promise.resolve().then(() => (init_LoadCustomPlugin(), LoadCustomPlugin_exports))
        }
      ]
    });
    registerSection({
